...
router.get('/candidates/:level', async (req, res) => {
  try {
    ...
    if (villageCellId) whereClause.villageCellId = villageCellId;
...
router.post('/candidates', async (req, res) => {
  try {
    ...
    const positionPath = `INTERNAL_PARTY.${level}.${category}.${position}`;
...
router.put('/candidates/:id', async (req, res) => {
  try {
    ...
    if (category || position) {
      const positionPath = `INTERNAL_PARTY.${participation.level}.${category || participation.category}.${position || participation.position}`;
...
router.get('/available-regions', async (req, res) => {
  try {
    const { level, category, position } = req.query;
    const whereClause = { level: level?.toUpperCase() || 'DISTRICT' };
...
router.get('/available-subregions', async (req, res) => {
  try {
    const { regionId, category, position } = req.query;
...
router.get('/available-districts', async (req, res) => {
  try {
    const { subregionId, category, position } = req.query;
    const whereClause = { subregionId };
    if (category) whereClause.category = category;
    if (position) whereClause.position = position;
    const participations = await CandidateParticipation.findAll({
      where: whereClause,
...
router.get('/available-constituencies', async (req, res) => {
  try {
    const { districtId, category, position } = req.query;
...
router.get('/available-subcounties', async (req, res) => {
  try {
    const { constituencyMunicipalityId, category, position } = req.query;
    const whereClause = { constituencyMunicipalityId };
    if (category) whereClause.category = category;
    if (position) whereClause.position = position;
    const participations = await CandidateParticipation.findAll({
      where: whereClause,
...
router.get('/available-parishes', async (req, res) => {
  try {
    const { subcountyDivisionId, category, position } = req.query;
    const whereClause = { subcountyDivisionId };
    if (category) whereClause.category = category;
    if (position) whereClause.position = position;
    const participations = await CandidateParticipation.findAll({
      where: whereClause,
...
router.get('/available-villages', async (req, res) => {
  try {
    const { parishWardId, category, position } = req.query;
    const whereClause = { parishWardId };
    if (category) whereClause.category = category;
    if (position) whereClause.position = position;
    const participations = await CandidateParticipation.findAll({
      where: whereClause,
